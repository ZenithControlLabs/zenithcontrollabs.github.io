...
async function updateDbgReporting() {
...
export function updateCanvas() {
    ...
    if (dbgCheckbox.checked) {
        ctx.fillStyle = "green";
        ctx.beginPath();
        const fx = -controllerState.fx * canvas.width;
        const fy = controllerState.fy * canvas.width;
        ctx.arc(fx, fy, stickCircRad/2, 0, 2*Math.PI);
        ctx.closePath();
        ctx.fill();
    }
